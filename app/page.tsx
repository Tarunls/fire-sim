...
import { 
  Terminal, AlertTriangle, Play, Pause, Loader2, Zap, ChevronRight, 
  Thermometer, PlusSquare, Droplets, Mountain, Compass, Clock, RefreshCw, 
  Activity, GraduationCap, Building2, LayoutDashboard, FileWarning, FireExtinguisher,
  Waves
} from 'lucide-react';
...
export default function IncidentCommander() {
  ...
  // --- SEARCH & FILTER STATE (LIFTED UP) ---
  const [searchTerm, setSearchTerm] = useState('');
  const [activeFilters, setActiveFilters] = useState<Set<string>>(
    new Set(['medical', 'power', 'response', 'school', 'civic', 'unknown'])
  );
  ...
  // --- 1. UNLIMITED GIS FETCH (BOUNDING BOX) ---
  useEffect(() => {
    const fetchInfrastructure = async () => {
      ...
      const query = `
        [out:json][timeout:90];
        (
          nwr["amenity"="hospital"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["healthcare"="hospital"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["amenity"="clinic"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["power"="substation"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["power"="generator"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["amenity"="fire_station"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["amenity"="school"](${latMin},${lonMin},${latMax},${lonMax});
          nwr["emergency"="designated"](${latMin},${lonMin},${latMax},${lonMax});
        );
        out center;
      `; 
  ...
  const { filteredMarkers, filteredRisks } = useMemo(() => {
    ...
    return {
      filteredMarkers: mapSource.filter(filterFn),
      filteredRisks: riskReport.filter(filterFn)
    };
      ...
      simHistory.forEach((frame, idx) => {
        frame.forEach((firePoint: any) => {
          ...
          landmarks.forEach(lm => {
            ...
            if (dist < 0.008) { 
               impactMap[lm.id] = { ...lm, timeToImpact: (idx * 0.5).toFixed(1), icon: getLandmarkIcon(lm.type, 16) };
            }
          });
        });
      });
  ...
  const heatmapLayer: any = {
    id: 'fire-heat', type: 'heatmap',
    paint: {
      'heatmap-weight': ['get', 'intensity'],
      'heatmap-color': ['interpolate', ['linear'], ['heatmap-density'], 0, 'rgba(0,0,0,0)', 0.1, 'rgba(50,0,0,0.5)', 0.3, 'rgb(100,0,0)', 0.6, 'rgb(255,100,0)', 1, 'rgb(255,255,200)'],
    ...
    <main className="bg-[#0a0e1a] text-slate-200 font-sans selection:bg-orange-500/30" style={{ display: 'grid', gridTemplateColumns: '400px 1fr', height: '100vh', width: '100vw', overflow: 'hidden' }}>
      ...
      <aside className="relative flex flex-col h-full bg-[#0B1121] border-r border-white/5 shadow-2xl z-20 overflow-hidden">
        ...
        <div className="p-6 border-b border-white/10 bg-[#0f172a]/80 backdrop-blur-sm relative z-10">
          ...
          <div className="flex bg-black/40 p-1 rounded-lg">
            ...
            <button onClick={() => setActiveTab('impact')} className={`flex-1 py-2 text-xs font-bold rounded-md transition-all flex items-center justify-center gap-2 ${activeTab === 'impact' ? 'bg-red-900/50 text-red-200 shadow-sm border border-red-500/20' : 'text-slate-500 hover:text-slate-300'}`}>
              <FileWarning size={14}/> IMPACT {riskReport.length > 0 && <span className="bg-red-500 text-white text-[9px] px-1.5 rounded-full animate-bounce">{riskReport.length}</span>}
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 relative z-10 custom-scrollbar">
            ...
            <div className="space-y-6 animate-in slide-in-from-left-4 fade-in duration-300">
               <div className="space-y-4">
                  ...
                  <div className="bg-white/5 rounded-xl p-3 border border-white/5">
                     <div className="flex justify-between text-xs mb-2"><span className="flex items-center gap-1"><Compass size={12}/> Wind Direction</span><span className="text-yellow-400 font-mono font-bold">{simParams.windDir}</span></div>
                     <div className="grid grid-cols-3 gap-1.5 mt-2">
                       {compassGrid.map((dir, idx) => (
                         <button key={idx} onClick={() => dir && setSimParams({...simParams, windDir: dir})} disabled={!dir} className={`h-8 text-[10px] font-bold rounded transition-all ${!dir ? 'invisible' : ''} ${simParams.windDir === dir ? 'bg-yellow-500 text-black shadow-[0_0_10px_rgba(234,179,8,0.4)] scale-105' : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white'}`}>{dir}</button>
                       ))}
                     </div>
                  </div>
               ...
               <div className="space-y-3 px-4 py-3 border border-white/5 bg-black/20 rounded-xl">
                 <div className="flex items-center justify-between"><label className="text-[10px] font-bold text-purple-400 uppercase tracking-widest flex items-center gap-2"><Terminal size={12} /> Neural Input</label>{aiMutation.isPending && <Loader2 size={12} className="text-purple-500 animate-spin" />}</div>
          ...
          {activeTab === 'impact' && (
            <RiskReport 
              risks={filteredRisks} // Pass filtered list
              totalCount={riskReport.length} // Pass raw count for badge
      ...
      <section className="relative h-full w-full bg-black z-10">
        <Map initialViewState={{ longitude: -121.5, latitude: 38.5, zoom: 11 }} style={{ width: '100%', height: '100%' }} mapStyle="mapbox://styles/mapbox/dark-v11" mapboxAccessToken={MAPBOX_TOKEN}>
          {filteredMarkers.map((lm: any) => { // Render Filtered Markers
          ...
          <div className="absolute bottom-8 left-8 right-8 bg-[#0f172a]/90 backdrop-blur-md border border-white/10 rounded-xl p-4 flex items-center gap-4 shadow-2xl z-50 animate-in fade-in slide-in-from-bottom-4">
            ...
            <div className="flex-1 flex flex-col gap-1">
              <div className="flex justify-between text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                ...
                <span className="text-orange-400 font-mono">EST: T+{ (currentFrame * 0.5).toFixed(1) } HRS</span>
                <span>T-{simParams.duration}:00</span>
              </div>
